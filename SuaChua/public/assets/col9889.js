...
function addFilteredItem(name, id) {
	var filteredItemTemplate = "<li class='filter-container__selected-filter-item' for='{3}'><a href='javascript:void(0)' onclick=\"{0}\"><i class='fa svg-close'></i> {1}</a></li>";
...
function doSearch(page, options) {
	...
	filter.search({
		...
		success: function (html) {
			...
			$('.filter-mb .aside-item').removeClass('active');
			$(".category-products").html($categoryProducts.html());
			pushCurrentFilterState({sortby: selectedSortby, page: page});
			awe_hidePopup('.loading');				  
			awe_lazyloadImage();
			$('.base-sidebar-collection .base-sort-category .base-sort-product .close-sort').click(function(){
				$('.base-sidebar-collection .base-sort-category').toggleClass('active');
			})
			$('html, body').animate({
				scrollTop: $('.category-products').offset().top
			}, 0);
			$('.item_product_main .add_to_cart').bind( 'click', addToCart );
			if(window.BPR)
